...
const port = 4600;
const urls = {
    products: `${protocol}://${hostname}:${port}/products`, orders: `${protocol}://${hostname}:${port}/orders`
};
export class RemoteDataSource extends AbstractDataSource {    
    ...
    storeOrder(): Promise<number> {        
        let orderData = {            
            lines: [...this.order.orderLines.values()]
                .map(ol => ({ productId: ol.product.id, productName: ol.product.name, quantity: ol.quantity }))       
        }
        return Axios.post<Product>(urls.orders, orderData).then(response => response.data.id);    
    }
}